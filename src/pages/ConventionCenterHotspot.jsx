...
// Mustard overlay along the bottom — Figma values for the convention
// hotspot pages: linear-gradient(180deg, rgba(176,141,102,0) 0%, #B08D66 76.5%)
const BOTTOM_BG =
  'linear-gradient(180deg, rgba(176, 141, 102, 0) 0%, #B08D66 76.5%)'
...
const ConventionCenterHotspot = () => {
  ...
  useEffect(() => {
    if (findConventionHotspotIndex(hotspot) === -1) {
      navigate(`/gallery/convention-center/${conventionCenterHotspots[0].slug}`, {
        replace: true,
      })
    }
  }, [hotspot, navigate])
  ...
  useEffect(() => {
    const idx = findConventionHotspotIndex(hotspot)
  ...
  useEffect(() => {
    ...
    if (!path) return
    const target = conventionCenterHotspots[activeIndex]?.progress ?? 0
  ...
  const goTo = (target) => {
    ...
    const total = conventionCenterHotspots.length
    let toIdx = target
    ...
    const onDone = () => {
      ...
      navigate(`/gallery/convention-center/${conventionCenterHotspots[toIdx].slug}`, {
        replace: true,
      })
    }
  ...
  return (
    ...
    >
      {conventionCenterHotspots.map((h, i) => (
        <div
          key={h.slug}
      ...
      {/* Bottom mustard gradient overlay — sits above slides, below UI. */}
      <div
        aria-hidden="true"
      ...
      <h1
        key={conventionCenterHotspots[activeIndex].slug}
        ref={titleRef}
      ...
      >
        {conventionCenterHotspots[activeIndex].name}
      </h1>
...
export default ConventionCenterHotspot
