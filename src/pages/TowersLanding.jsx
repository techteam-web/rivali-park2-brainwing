...
// Pill placement on the aerial, as fractions of the render. The image is
// cover-cropped (see the sizing wrapper below), so on wide 16:9 screens roughly
// the top/bottom ~15% is clipped. Keep every pill inside the always-visible
// central band (~0.18-0.82) so none disappear at landscape aspect ratios — the
// same reason the gallery's own tower pills sit at 0.235-0.775. Keyed by id.
const PILL_POS = {
  skyleap: { left: 0.42, top: 0.19 },
  sunburst: { left: 0.92, top: 0.38 },
  moonrise: { left: 0.61, top: 0.81 },
  stargaze: { left: 0.91, top: 0.81 },
}

const PILL_CLASS =
  'absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full border border-white/40 font-normal text-white text-center backdrop-blur-[1.3px] xl:backdrop-blur-[1.6px] 2xl:backdrop-blur-[2px] 3xl:backdrop-blur-[2.5px] 4xl:backdrop-blur-[3.3px] 5xl:backdrop-blur-[5px] transition-[transform,filter] duration-200 hover:scale-[1.05] hover:brightness-110 focus:outline-none focus-visible:scale-[1.05] focus-visible:brightness-110 px-5 py-1.5 text-[13px] leading-[165%] xl:px-6.25 xl:py-2 xl:text-[16px] 2xl:px-7.5 2xl:py-2.25 2xl:text-[20px] 3xl:px-9.5 3xl:py-2.75 3xl:text-[24px] 4xl:px-12.5 4xl:py-3.75 4xl:text-[32px] 5xl:px-18.75 5xl:py-5.75 5xl:text-[49px]'
