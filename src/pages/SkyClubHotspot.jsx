...
// Green overlay along the bottom — Figma values for the sky-club hotspot
// pages: linear-gradient(180deg, rgba(129,149,120,0) 0%, #819578 76.5%)
const BOTTOM_BG =
  'linear-gradient(180deg, rgba(129, 149, 120, 0) 0%, #819578 76.5%)'
...
const SkyClubHotspot = () => {
  ...
  useEffect(() => {
    if (findSkyHotspotIndex(hotspot) === -1) {
      navigate(`/gallery/sky-club/${skyClubHotspots[0].slug}`, {
        replace: true,
      })
    }
  }, [hotspot, navigate])
  ...
  useEffect(() => {
    const idx = findSkyHotspotIndex(hotspot)
  ...
  useEffect(() => {
    ...
    if (!path) return
    const target = skyClubHotspots[activeIndex]?.progress ?? 0
  ...
  const goTo = (target) => {
    ...
    const total = skyClubHotspots.length
    let toIdx = target
    ...
    const onDone = () => {
      ...
      navigate(`/gallery/sky-club/${skyClubHotspots[toIdx].slug}`, {
        replace: true,
      })
    }
  ...
  return (
    ...
    >
      ...
      {/* Bottom green gradient overlay — sits above slides, below UI. */}
      <div
        aria-hidden="true"
      ...
      <h1
        key={skyClubHotspots[activeIndex].slug}
        ref={titleRef}
      ...
      >
        {skyClubHotspots[activeIndex].name}
      </h1>
