...
// Brown overlay along the bottom — Figma values for the hotspot pages:
// linear-gradient(180deg, rgba(122,72,51,0) 0%, #7A4833 76.5%)
const BOTTOM_BG =
  'linear-gradient(180deg, rgba(122, 72, 51, 0) 0%, #7A4833 76.5%)'

// Wavy path used by both the progress bar's track and its filled overlay.
// Lifted from public/gallery/hotspots/social club/progress bar.svg so the
// two layers can be addressed independently for drawSVG without parsing
// the source SVG's mask structure.
...
// Single-page slider for Social Club hotspot detail screens. The page is
// driven entirely by the `socialClubHotspots` config — adding a new entry
// there gets a new slide for free. Wheel / touch / arrow keys advance
// between slides; the URL is kept in sync so links into a specific hotspot
// (e.g. from the parent map page) land on the right slide.
const SocialClubHotspot = () => {
  ...
  // If the slug doesn't match any configured hotspot, redirect to the
  // first valid one rather than rendering an empty page.
  useEffect(() => {
    if (findHotspotIndex(hotspot) === -1) {
      navigate(`/gallery/social-club/${socialClubHotspots[0].slug}`, {
        replace: true,
      })
    }
  }, [hotspot, navigate])

  // Keep activeIndex in sync if the slug changes via direct URL navigation
  // (e.g. browser back/forward). Internal nav already calls setActiveIndex.
  useEffect(() => {
    const idx = findHotspotIndex(hotspot)
  ...
  // (0..1). On first mount we snap the bar to empty, then tween up to
  // the target — landing on any hotspot starts the bar at zero and draws
  // it in. Subsequent slide changes just tween from current to target.
  const hasMountedProgressRef = useRef(false)
  useEffect(() => {
    const path = progressRef.current
    if (!path) return
    const target = socialClubHotspots[activeIndex]?.progress ?? 0
  ...
  const goTo = (target) => {
    ...
    const total = socialClubHotspots.length
    let toIdx = target
    ...
    const onDone = () => {
      ...
      navigate(`/gallery/social-club/${socialClubHotspots[toIdx].slug}`, {
        replace: true,
      })
    }
  ...
  // a single trackpad gesture from triggering multiple transitions.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Bottom brown gradient overlay — sits above slides, below UI.
      ...
      >
        {socialClubHotspots[activeIndex].name}
      </h1>
...
export default SocialClubHotspot
