...
const StatCard = ({ label, value, accent, valueClassName = '' }) => (
  <div
    data-stat-card
    className="flex-1 basis-0 min-w-0 px-6 md:px-2 lg:px-1 xl:px-1 2xl:px-5 3xl:px-6 4xl:px-8 5xl:px-12 py-5 md:py-3 2xl:py-4 3xl:py-5 4xl:py-7 5xl:py-10 rounded-md"
    style={{ backgroundColor: `${accent}0A` }}
  >
    <p className="font-sans font-medium text-[15px] md:text-[12px] lg:text-[13px] xl:text-[14px] 2xl:text-[16px] 3xl:text-[19px] 4xl:text-[25px] 5xl:text-[38px] tracking-[2px] uppercase text-on-light-grey mb-1">
      {label}
    </p>
    <p
      data-reveal
      className={`font-sans font-medium text-[26px] md:text-[20px] lg:text-[20px] xl:text-[26px] whitespace-nowrap 2xl:text-[30px] 3xl:text-[30px] 4xl:text-[40px] 5xl:text-[60px] leading-tight text-on-light-black ${valueClassName}`}
    >
      {value}
    </p>
  </div>
    ...
    <div ref={ref} className="absolute inset-0 z-30 pointer-events-none" data-tower-id={tower.id}>
      <div className="grid grid-cols-12 gap-0 items-stretch h-full">
        ...
        >
          ...
          <div className="flex gap-4 md:gap-2 xl:gap-3 3xl:gap-4 4xl:gap-5 5xl:gap-8 lg:ml-3 xl:ml-3">
